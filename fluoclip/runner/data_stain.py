...
from fluoclip.utils.logging import get_logger, print_log
...
class RegressionDataset(Dataset):
    ...
    def __getitem__(self, index):
        img_file, target_list, stain_list = self.images_file[index], self.labels[index], self.stains[index]
        if "val" in self.name or "test" in self.name:
    ...
    def split_dataset_by_label_and_stain(self):
        output = defaultdict(list)
        for img, label, stain in zip(self.images_file, self.labels, self.stains):
            target = label[len(label) // 2]
            key = (target, stain)   # <-- label + stain 조합을 key 로 사용
            output[key].append(img)
        return output
    ...
    def generate_fewshot_dataset(self, num_shots=-1, repeat=False):
        ...
        output = self.split_dataset_by_label_and_stain()

        print("generate few-shot dataset")
        print("clear full dataset: images_file & labels")
        self._images_file = self.images_file
        self._labels = self.labels
        self._stains = self.stains

        self.images_file = []
        self.labels = []
        self.stains = []

        print(f"build few_shot by label/stain: {len(output.keys())} classes")

        for (label, stain), imgs_ls in output.items():
            if len(imgs_ls) >= num_shots:
                sampled_imgs_ls = random.sample(imgs_ls, num_shots)
            else:
                print(f"not enough: class-(label={label}, stain={stain}): {len(imgs_ls)}")
                if repeat:
                    sampled_imgs_ls = random.choices(imgs_ls, k=num_shots)
                else:
                    sampled_imgs_ls = imgs_ls

            for img in sampled_imgs_ls:
                idx = self._images_file.index(img)
                self.images_file.append(img)
                self.labels.append([label])
                self.stains.append(stain)

        assert len(self.images_file) == len(self.labels) == len(self.stains)
    ...
    #         for img in sampled_imgs_ls:
    #             idx = self._images_file.index(img)  # find original index
    #             self.images_file.append(img)
    #             self.labels.append([label])
    #             self.stains.append(self._stains[idx])
            
    #     assert len(self.images_file) == len(self.labels) == len(self.stains), \
    #     f"dataset mismatch: {len(self.images_file)}, {len(self.labels)}, {len(self.stains)}"
    #     print(f"len of few shot dataset: {len(self.images_file)}")
    ...
    def generate_distribution_shifted_dataset(self, num_topk_scaled_class=-1, scale_factor=0.3):
        ...
        for idx, label_cnt in enumerate(num_samples_per_label):
            ...
            for img in sampled_imgs_ls:
                idx = self._images_file.index(img)   # original index
                self.images_file.append(img)
                self.labels.append([label_cnt[0]])
                self.stains.append(self._stains[idx])
        assert len(self.images_file) == len(self.labels) == len(self.stains), f"{len(self.images_file)} != {len(self.labels)} != {len(self.stains)}"
