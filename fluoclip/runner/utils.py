...
class SelectLargestChannel(object):
    def __call__(self, img):
        # img: Tensor (C,H,W) or PIL Image
        if not torch.is_tensor(img):
            img = transforms.ToTensor()(img)

        # Identify channel with largest sum
        channel_sums = img.view(3, -1).sum(dim=1)
        largest_channel_idx = channel_sums.argmax().item()

        # Select only the largest channel
        single_channel_img = img[largest_channel_idx].unsqueeze(0)

        # Repeat back to 3 channels (normalize는 pipeline에서 따로 적용됨)
        single_channel_img = single_channel_img.repeat(3, 1, 1)
...
def get_transforms(
    ...
    if "largest_channel" in input_transforms:
        transform_train.append(SelectLargestChannel())
        transform_test.append(SelectLargestChannel())
...
def set_requires_grad(module, requires_grad):
    if isinstance(module, nn.Module):
        ...
        # 3) ParameterList / ModuleList
    elif isinstance(module, (nn.ParameterList, nn.ModuleList)):
        for x in module:
            set_requires_grad(x, requires_grad)
        return
    # 4) dict 컨테이너
    elif isinstance(module, dict):
        for x in module.values():
            set_requires_grad(x, requires_grad)
        return

    # 5) Iterable (list, tuple, set, generator 등)
    elif isinstance(module, abc.Iterable):
        for x in module:
            set_requires_grad(x, requires_grad)
        return
    else:
